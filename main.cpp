...
int main(int argc, char **argv)
{
    ...
    const char *seed_env = std::getenv("SVD_SEED");
    const long long base_seed = (argc >= 2) ? std::stoll(argv[1])
                                            : (seed_env ? std::stoll(seed_env) : 20260408LL);

    const char *backend_env = std::getenv("SVD_BACKEND");
    const bool use_cuda_bidiag = backend_env && std::string(backend_env) == "cuda";

#ifdef USE_CUDA_BIDIAG
    std::cout << "Bidiagonalization backend: "
              << (use_cuda_bidiag ? "CUDA/cuBLAS" : "CPU")
              << "\n";
#else
    ...
    // 样例1：5x5 固定值矩阵
    {
        ...
        A.at(4, 4) = 4.0;
        ++total;
        if (run_case("固定值 5x5", A, sum_bidiag_ms, sum_gkh_ms, use_cuda_bidiag))
        {
            ++passed;
        }
    }
    ...
    // 样例2：8x8 随机矩阵
    {
        ...
        ++total;
        if (run_case("随机 8x8", A, sum_bidiag_ms, sum_gkh_ms, use_cuda_bidiag))
        {
            ++passed;
        }
    }
    ...
    // 构造方式：先随机生成，再让第3列接近第1列，以测试对近相关列的稳定性。
    {
        ...
        ++total;
        if (run_case("近秩亏损 10x8", A, sum_bidiag_ms, sum_gkh_ms, use_cuda_bidiag))
        {
            ++passed;
        }
    }
    ...
    // 样例4：10x8 随机矩阵
    {
        ...
        ++total;
        if (run_case("随机 10x8", A, sum_bidiag_ms, sum_gkh_ms, use_cuda_bidiag))
        {
            ++passed;
        }
    }
    ...
    // 样例5：大规模 1000x1000 随机矩阵
    {
        ...
        ++total;
        if (run_case("随机 1000x1000", A, sum_bidiag_ms, sum_gkh_ms, use_cuda_bidiag))
        {
            ++passed;
        }
    }
